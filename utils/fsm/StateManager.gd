...
func _ready():
	...
	if initial_state:
		initial_state._on_state_enter()
		current_state = initial_state

func _process(delta):
	if current_state:
		current_state._on_state_update(delta)

func _physics_process(delta):
	if current_state:
		current_state._on_state_physics_update(delta)
...
func transition_to_state(next_state: State):
	current_state.transition_to_state(next_state)
